...
export class FixProductionBatches1740960000005 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_production_batches_product') THEN ALTER TABLE "production_batches" ADD CONSTRAINT "FK_production_batches_product" FOREIGN KEY ("product_id") REFERENCES "products"("id") ON DELETE NO ACTION ON UPDATE NO ACTION; END IF; END $$;`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_batch_components_raw_material') THEN ALTER TABLE "batch_components" ADD CONSTRAINT "FK_batch_components_raw_material" FOREIGN KEY ("raw_material_id") REFERENCES "raw_materials"("id") ON DELETE NO ACTION ON UPDATE NO ACTION; END IF; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_batch_components_accessory') THEN ALTER TABLE "batch_components" ADD CONSTRAINT "FK_batch_components_accessory" FOREIGN KEY ("accessory_id") REFERENCES "accessories"("id") ON DELETE NO ACTION ON UPDATE NO ACTION; END IF; END $$;`,
    );
  }

...
export class JobController {
  ...
  @Put(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body() dto: Partial<CreateJobDto>) { return this.service.update(id, dto); }

  @Post(':id/costs')
  addCost(@Param('id', ParseIntPipe) id: number, @Body() dto: Omit<CreateJobCostDto, 'job_id'>) {
    return this.service.addCost({ ...dto, job_id: id });
  }

  @Get(':id/profitability')
  getProfitability(@Param('id', ParseIntPipe) id: number) { return this.service.getProfitability(id); }
}

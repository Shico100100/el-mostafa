...
export class TimeBillingController {
  ...
  @Get('unbilled')
  getUnbilled() {
    return this.service.getUnbilled();
  }

  @Post('bill')
  markBilled(@Body() body: { ids: number[] }) {
    return this.service.markBilled(body.ids);
  }

  @Get('summary/:jobId')
  getSummary(@Param('jobId', ParseIntPipe) jobId: number) {
    return this.service.getSummaryByJob(jobId);
  }
}

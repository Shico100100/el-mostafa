...
export class TimeBillingService {
  ...
  async findAll(filters?: { job_id?: number; user_id?: number; start_date?: string; end_date?: string }): Promise<TimeEntry[]> {
    const where: any = {};
    if (filters?.job_id) where.job_id = filters.job_id;
    if (filters?.user_id) where.user_id = filters.user_id;
    return this.timeRepo.find({ where, relations: ['job', 'phase', 'user'], order: { date: 'DESC' } });
  }

  async findOne(id: number): Promise<TimeEntry> {
    const entry = await this.timeRepo.findOne({ where: { id }, relations: ['job', 'phase', 'user'] });
    if (!entry) throw new NotFoundException(`Time entry #${id} not found`);
    return entry;
  }

  async create(dto: CreateTimeEntryDto): Promise<TimeEntry> {
    return this.timeRepo.save(this.timeRepo.create({
      ...dto,
  ...
  async update(id: number, dto: Partial<CreateTimeEntryDto>): Promise<TimeEntry> {
  ...
  async getUnbilled(): Promise<TimeEntry[]> {
    return this.timeRepo.find({ where: { is_billed: false, is_billable: true }, relations: ['job', 'phase', 'user'], order: { date: 'ASC' } });
  }
  ...
  async getSummaryByJob(jobId: number): Promise<any> {
    const entries = await this.timeRepo.find({ where: { job_id: jobId }, relations: ['phase'] });
    const totalHours = entries.reduce((s, e) => s + Number(e.hours), 0);
    const billableHours = entries.filter(e => e.is_billable).reduce((s, e) => s + Number(e.hours), 0);
    const totalBillable = entries.filter(e => e.is_billable).reduce((s, e) => s + Number(e.hours) * Number(e.billing_rate), 0);
    ...
    const byPhase = entries.reduce((acc, e) => {
      ...
      if (e.is_billable) acc[key].billable += Number(e.hours) * Number(e.billing_rate);
      return acc;
    }, {} as Record<string, { hours: number; billable: number }>);

    return { job_id: jobId, totalHours, billableHours, totalBillable, byPhase, entryCount: entries.length };
  }
}

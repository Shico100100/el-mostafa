...
export class FixedAssetService {
  constructor(
    ...
    private deprRepo: Repository<DepreciationEntry>,
  ) {}
  ...
  async findOne(id: number): Promise<FixedAsset> {
    const asset = await this.assetRepo.findOne({
      where: { id },
      relations: ['depreciation_entries'],
    });
  ...
  async create(dto: CreateFixedAssetDto): Promise<FixedAsset> {
    const asset = this.assetRepo.create({
      ...
      depreciation_method:
        dto.depreciation_method || DepreciationMethod.STRAIGHT_LINE,
    });
  ...
  async update(
    id: number,
    dto: Partial<CreateFixedAssetDto>,
  ): Promise<FixedAsset> {
  ...
  async depreciate(id: number, period: string): Promise<DepreciationEntry> {
    ...
    if (asset.status !== AssetStatus.ACTIVE)
      throw new BadRequestException('الأصل غير نشط');
    if (asset.book_value <= asset.salvage_value)
      throw new BadRequestException('الأصل محول بالكامل');

    const existing = await this.deprRepo.findOne({
      where: { asset_id: id, period },
    });
    if (existing)
      throw new BadRequestException(`الإهلاك مسجل بالفعل للفترة ${period}`);

    let deprAmount = 0;
    if (asset.depreciation_method === DepreciationMethod.STRAIGHT_LINE) {
      deprAmount =
        (Number(asset.purchase_cost) - Number(asset.salvage_value)) /
    ...
    deprAmount = Math.min(
      deprAmount,
      Number(asset.book_value) - Number(asset.salvage_value),
    );
    deprAmount = Math.round(deprAmount * 100) / 100;

    asset.accumulated_depreciation =
      Number(asset.accumulated_depreciation) + deprAmount;
    asset.book_value = Number(asset.book_value) - deprAmount;
    await this.assetRepo.save(asset);

    return this.deprRepo.save(
      this.deprRepo.create({
        asset_id: id,
        period,
        amount: deprAmount,

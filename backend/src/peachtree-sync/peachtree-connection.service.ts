...
export class PeachtreeConnectionService {
  ...
  constructor(private configService: ConfigService) {
    // eslint-disable-next-line no-restricted-syntax -- untyped config: explicit generic + default is the correct form
    this.defaultDataPath = this.configService.get<string>(
      'PEACHTREE_DATA_PATH',
      'mos',
    );
  ...
  async query(
    ...
    if (this.cacheEnabled && this.queryCache.has(cacheKey)) {
      this.logger.log(
        `Cache hit: ${table} (${this.queryCache.get(cacheKey)!.length} rows)`,
      );
    ...
    this.logger.log(
      `Querying Peachtree table: ${table}${where ? ` (${where})` : ''}`,
    );
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          `Failed to parse JSON output: ${output.substring(0, 200)}`,
        );
        throw new Error(
          `Peachtree query failed for table ${table}: ${output.substring(0, 500)}`,
        );
      }
      ...
      if (retries > 0 && error?.message?.includes('too many clients')) {
        ...
        this.logger.warn(
          `Connection pool exhausted for ${table}, retrying in ${delay}ms (${retries} retries left)`,
        );
  ...
  private async ensurePervasiveRunning(): Promise<void> {
    ...
    for (const svc of serviceNames) {
      try {
        const { stdout } = await execFileAsync('sc', ['query', svc], {
          timeout: 10000,
          encoding: 'utf8',
        });
        ...
        this.logger.log(
          `Pervasive service "${svc}" found but not running, starting...`,
        );
        await execFileAsync('net', ['start', svc], {
          timeout: 10000,
          encoding: 'utf8',
        });
  ...
  private async resolveDatabaseName(input: string): Promise<string> {
    ...
    if (this.resolvedDatabaseName) {
      this.logger.log(
        `Using cached resolved database name: "${this.resolvedDatabaseName}"`,
      );
      return this.resolvedDatabaseName;
    }

    this.logger.log(
      `Input "${trimmed}" looks like a file path, resolving Pervasive database name...`,
    );
    ...
    this.logger.log(
      `Falling back to default database name: "${this.defaultDataPath}"`,
    );
  ...
  async testConnection(): Promise<{ connected: boolean; error?: string }> {
    ...
    if (resolvedDb !== this.dataPath) {
      this.logger.log(
        `Auto-corrected database name: "${this.dataPath}" -> "${resolvedDb}"`,
      );
    ...
    try {
      ...
      this.logger.log(
        `Peachtree connection test passed - GLAccount rows: ${rows.length}`,
      );

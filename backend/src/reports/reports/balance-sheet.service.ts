...
export class BalanceSheetService {
  ...
  async generate(asOfDate?: string): Promise<any> {
    ...
    return {
      ...
      assets: { items: assets.filter(a => a.balance !== 0), total: totalAssets },
      liabilities: { items: liabilities.filter(l => l.balance !== 0), total: totalLiabilities },
      equity: { items: equity.filter(e => e.balance !== 0), total: totalEquity },

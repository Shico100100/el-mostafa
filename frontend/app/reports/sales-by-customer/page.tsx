    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f0d] via-[#0f1714] to-[#0a0f0d]" dir="rtl">
        ...
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white flex items-center gap-3">
              <Users className="w-7 h-7 text-emerald-400" />مبيعات حسب العميل
            </h1>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row gap-4 mb-6 justify-between items-end">
          <div className="flex flex-col md:flex-row gap-4 flex-1">
            <div>
              ...
              <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                className="px-4 py-2 bg-[#121a16] border border-white/20 rounded-lg text-white" />
            </div>
            <div>
          ...
          <div className="flex gap-2">
            <button onClick={exportCsv}
              className="px-4 py-2 h-[42px] bg-[#121a16] hover:bg-white/20 text-white rounded-lg border border-white/20 transition flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div id="sales-by-customer-content" className="p-6 bg-[#0a0f0d] rounded-xl">
            {data?.period && (
              <p className="text-[#6b8378] text-sm mb-4">الفترة: {data.period.start} — {data.period.end}</p>
            )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {!data?.customers?.length ? (
                      <tr><td colSpan={5} className="py-12 text-center text-[#6b8378]">لا توجد بيانات مبيعات</td></tr>
                    ) : (
                      <>
                        {data.customers.map((c: SalesByCustomerRow, i: number) => (
                          <tr key={i} className="border-b border-[#1f2d26] hover:bg-[#121a16] transition">
                            ...
                            <td className="py-3 px-4 text-emerald-400">{c.order_count}</td>
                            <td className="py-3 px-4 text-green-400 font-bold">{Number(c.total_amount).toLocaleString()} ج.م</td>
                            <td className="py-3 px-4 text-[#ecfdf5]">{c.first_order ? new Date(c.first_order).toLocaleDateString('ar-EG') : '-'}</td>
                            <td className="py-3 px-4 text-[#ecfdf5]">{c.last_order ? new Date(c.last_order).toLocaleDateString('ar-EG') : '-'}</td>
                          </tr>
                        ))}
                        <tr className="bg-[#121a16] border-t border-white/20">
                          <td className="py-3 px-4 text-white font-bold">الإجمالي</td>
                          <td className="py-3 px-4 text-emerald-400 font-bold">{data.customers.reduce((s: number, c: SalesByCustomerRow) => s + c.order_count, 0)}</td>

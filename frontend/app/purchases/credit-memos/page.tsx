'use client';
import { usePurchaseCreditMemos } from '@/hooks/purchases/usePurchaseCreditMemos';
import { Receipt } from 'lucide-react';

export default function PurchaseCreditMemosPage() {
  const h = usePurchaseCreditMemos();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900" dir="rtl">
      <div className="container mx-auto px-6 py-8">
        <h1 className="text-3xl font-bold text-white flex items-center gap-3 mb-2"><Receipt className="w-8 h-8 text-orange-400" />الإشعارات الدائنة - المشتريات</h1>
        <p className="text-gray-400 mb-8">إدارة إشعارات الدائنة للموردين</p>

        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-gray-400 border-b border-white/10">
                <th className="py-3 px-4 text-right">رقم</th><th className="py-3 px-4 text-right">التاريخ</th><th className="py-3 px-4 text-right">المورد</th><th className="py-3 px-4 text-right">المبلغ</th><th className="py-3 px-4 text-right">السبب</th><th className="py-3 px-4 text-right">الحالة</th>
              </tr></thead>
              <tbody>
                {h.memos.length === 0 ? (
                  ...
                  <tr key={m.id} className="border-b border-white/5 hover:bg-white/5 transition">
                    ...
                    <td className="py-3 px-4 text-white">{m.supplier?.name || '-'}</td>

  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f0d] via-[#0f1714] to-[#0a0f0d]" dir="rtl">
      <div className="container mx-auto px-6 py-8">
        ...
        <p className="text-[#6b8378] mb-8">إدارة إغلاق وفتح الفترات المحاسبية الشهرية</p>
        ...
        <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl p-6 mb-8">
          ...
          <div className="flex gap-4 items-end">
            <div><label className="text-[#6b8378] text-sm">الفترة</label><input type="month" value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)} className="w-full mt-1 px-3 py-2 bg-[#121a16] border border-[#1f2d26] rounded-lg text-white" /></div>
        ...
        <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {h.periods.length === 0 ? (
                  <tr><td colSpan={5} className="py-12 text-center text-[#6b8378]">لا توجد فترات مسجلة</td></tr>
                ) : h.periods.map((p) => (
                  <tr key={p.id} className="border-b border-[#1f2d26]">
                    ...
                    <td className="py-3 px-4 text-[#ecfdf5]">{p.closed_by || '-'}</td>
                    <td className="py-3 px-4 text-[#6b8378]">{p.closed_at ? new Date(p.closed_at).toLocaleString('ar') : '-'}</td>

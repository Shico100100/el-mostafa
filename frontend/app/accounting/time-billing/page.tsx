  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f0d] via-[#0f1714] to-[#0a0f0d]" dir="rtl">
      <div className="container mx-auto px-6 py-8">
        ...
        <p className="text-[#6b8378] mb-8">تتبع ساعات العمل والحساب عليها</p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl p-6"><p className="text-[#6b8378] text-sm">إجمالي السجلات</p><p className="text-2xl font-bold text-white mt-1">{h.entries.length} سجل</p></div>
          <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl p-6"><p className="text-[#6b8378] text-sm">ساعات غير مفوترة</p><p className="text-2xl font-bold text-yellow-400 mt-1">{totalUnbilledHours} ساعة</p></div>
          <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl p-6"><p className="text-[#6b8378] text-sm">قيمة غير مفوترة</p><p className="text-2xl font-bold text-green-400 mt-1">{totalUnbilledValue.toLocaleString()} ج.م</p></div>
        </div>
        ...
        <div className="bg-black/40 backdrop-blur-xl border border-[#1f2d26] rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {h.entries.length === 0 ? (
                  <tr><td colSpan={6} className="py-12 text-center text-[#6b8378]">لا توجد سجلات وقت</td></tr>
                ) : h.entries.map((e) => (
                  <tr key={e.id} className="border-b border-[#1f2d26] hover:bg-[#121a16] transition">
                    <td className="py-3 px-4 text-white">{new Date(e.date).toLocaleDateString('ar')}</td>
                    <td className="py-3 px-4 text-[#ecfdf5]">{e.job?.name || '-'}</td>
                    <td className="py-3 px-4 text-teal-400 font-semibold">{e.hours} س</td>
                    <td className="py-3 px-4 text-[#6b8378] text-xs max-w-[200px] truncate">{e.description || '-'}</td>
                    <td className="py-3 px-4">{e.is_billable ? <Timer className="w-4 h-4 text-green-400" /> : <span className="text-[#6b8378]">-</span>}</td>

...
export function BOMCreateDialog(props: BOMCreateDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-800 border border-white/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
        ...
        <div className="px-6 py-4 border-t border-white/10 flex justify-end gap-3">
          ...
          <button onClick={props.onSave} className="px-4 py-2 bg-emerald-500/20 text-emerald-300 rounded-lg border border-emerald-500/30 hover:bg-emerald-500/30 transition">حفظ</button>
        </div>
      </div>
    </div>
  );

...
import { LayoutDashboard, Package, ClipboardList, ArrowRightLeft, ArrowLeft, Warehouse, GitCompareArrows, CircleDot, Percent, SlidersHorizontal } from 'lucide-react';

const mainLinks = [
  { href: '/inventory2', label: 'لوحة التحكم', icon: LayoutDashboard },
  { href: '/inventory2/products', label: 'المنتجات', icon: Package },
  { href: '/inventory2/semi-finished', label: 'البلاستيك', icon: CircleDot },
...
const managementLinks = [
  { href: '/inventory2/warehouses', label: 'المخازن', icon: Warehouse },
];

const actionsLinks = [
  { href: '/inventory2/products/bulk-prices', label: 'تحديث الأسعار', icon: Percent },
  { href: '/inventory2/stock/transfer', label: 'تحويل مخزني', icon: GitCompareArrows },
  { href: '/inventory2/stock/adjust', label: 'تسوية المخزون', icon: SlidersHorizontal },
];
  ...
  return (
    <aside className="w-64 bg-slate-900/80 backdrop-blur-xl border-l border-white/10 min-h-screen flex flex-col shrink-0">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        <p className="text-[10px] text-slate-600 uppercase tracking-widest px-4 mb-2 mt-1">رئيسي</p>
        {mainLinks.map((link) => {
        ...
        <p className="text-[10px] text-slate-600 uppercase tracking-widest px-4 mb-2 mt-4">إدارة</p>
        ...
        <p className="text-[10px] text-slate-600 uppercase tracking-widest px-4 mb-2 mt-4">عمليات</p>

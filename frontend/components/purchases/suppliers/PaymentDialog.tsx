...
export function PaymentDialog({ visible, supplier, onSave, onClose }: PaymentDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-slate-800 p-8 rounded-2xl w-full max-w-md border border-white/20" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6">دفع مبلغ لـ: {supplier?.name}</h2>

    ...
    <div style={{ display: 'none' }}>
      <div ref={ref} className="p-12 text-right" dir="rtl">
        ...
        <div className="grid grid-cols-2 gap-8 mb-12">
          <div className="bg-[#ecfdf5] p-6 rounded-2xl"><h3 className="font-black text-[#ecfdf5]0 mb-2 border-b border-slate-300 pb-1">بيانات العميل</h3><p className="text-2xl font-black">{order?.customer?.name}</p><p className="mt-1">{order?.customer?.phone}</p><p>{order?.customer?.address}</p></div>
          <div className="bg-[#ecfdf5] p-6 rounded-2xl flex flex-col justify-center text-center border-R-4 border-slate-900"><p className="text-[#ecfdf5]0 font-bold mb-1">إجمالي الفاتورة</p><p className="text-4xl font-black">{Number(order?.total_amount).toLocaleString()} ج.م</p></div>
        </div>

...
export function RawMaterialsToolbar({ onEntryLog, onConsumption, onAdd }: RawMaterialsToolbarProps) {
  return (
    <div className="flex gap-4 mb-6">
      <button onClick={onEntryLog} className="px-4 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-blue-300 rounded-lg font-semibold transition flex items-center gap-2">
        <FileText /> سجل دخول الخامات
      </button>
      <button onClick={onConsumption} className="px-4 py-2 bg-teal-500/20 hover:bg-teal-500/30 text-purple-300 rounded-lg font-semibold transition flex items-center gap-2">

...
const links = [
  { href: '/inventory', label: 'لوحة التحكم', icon: LayoutDashboard },
  { href: '/inventory/products', label: 'المنتجات', icon: Package },
  { href: '/inventory/stock', label: 'المخزون', icon: ClipboardList },
  { href: '/inventory/stock/movements', label: 'الحركات', icon: ArrowRightLeft },
];
  ...
  return (
    <aside className="w-64 bg-slate-900/80 backdrop-blur-xl border-l border-white/10 min-h-screen flex flex-col shrink-0">
      ...
      <nav className="flex-1 p-4 space-y-1">
        {links.map((link) => {
          const active = pathname === link.href || (link.href !== '/inventory' && pathname.startsWith(link.href));
          return (
            <button
              key={link.href}

...
export function LowStockPanel() {
  ...
  if (loading) return <div className="text-gray-500 text-sm text-center py-4">جاري التحميل...</div>;
  ...
  return (
    <div>
      ...
      {products.length === 0 ? (
        <p className="text-gray-500 text-xs text-center py-3">لا يوجد نقص في المخزون</p>
      ) : (
          ...
          {products.slice(0, 5).map((p) => (
            <div key={p.id} className="flex justify-between items-center py-1.5 px-3 bg-red-500/5 border border-red-500/10 rounded-lg">
              <div>
                ...
                {p.sku && <span className="text-[10px] text-gray-500 mr-2">#{p.sku}</span>}
              </div>
              <div className="text-[11px]">
                <span className="text-red-400 font-bold">{p.stock_quantity}</span>
                <span className="text-gray-600 mx-1">/</span>
                <span className="text-gray-500">{p.min_stock}</span>

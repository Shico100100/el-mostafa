    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="glass rounded-2xl border border-white/10 w-full max-w-lg mx-4 max-h-[80vh] flex flex-col shadow-2xl">
            ...
            >
              <div className="flex flex-col gap-1">
                <button
                  ...
                  disabled={idx === 0}
                  className="text-[#ecfdf5]0 hover:text-white disabled:opacity-30 transition p-0.5"
                >
                ...
                <div className="flex gap-1 mt-1.5">
                  {([1, 2, 3] as const).map((col) => (
                    <button
                      ...
                      className={`text-[10px] px-2 py-0.5 rounded-md transition ${panel.column === col ? 'bg-emerald-500/20 text-blue-300 border border-emerald-500/30' : 'bg-white/5 text-[#ecfdf5]0 border border-white/5 hover:text-slate-300'}`}
                    >
                      عمود {col}
                    </button>
                  ))}
              ...
              <button
                ...
                className={`relative w-10 h-5 rounded-full transition-all ${panel.visible ? 'bg-emerald-500' : 'bg-white/10'}`}
              >
                <span
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/10">
          ...
          <button
            onClick={() => setIsCustomizing(false)}
            className="px-5 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-blue-200 rounded-xl text-sm font-medium transition border border-emerald-500/20"
          >
            تم
          </button>
        </div>
